...
export const getSinglePage = async (folder) => {
  if (folder === "content/blog" || folder === "content/blog/") {
    try {
      process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0'; // Bypass local SSL error
      ...
      if (res.ok) {
        ...
        const filterByDate = publishedPages; // Removed date filter temporarily for debugging
        return filterByDate;
      } else {
        console.error("Failed to fetch blogs from DB: ", res.status);
      }
    } catch (e) {
      console.error("Exception fetching blogs from DB", e);
    }
  }
