...
const Chatbot = () => {
  ...
  const handleSendMessage = (e) => {
    ...
    setTimeout(() => {
      ...
      // Intercept demo length/meeting duration query specifically to avoid starting booking questionnaire
      if (lowerText.includes("length") || lowerText.includes("how long") || lowerText.includes("duration")) {
        ...
        setMessages((prev) => [
          ...
          {
            sender: "bot",
            text: "A standard live walk-through with our security engineers takes between **15 to 20 minutes**. We will showcase workspace navigation, simulation templates, SCORM integrations, and employee reporting dashboards. You can request a callback on our [Book a Demo page](/book-demo) or book here by typing 'book a demo'."
          }
        ]);
        return;
      }
      ...
      // Check pricing and quote queries
      if (
        ...
        setMessages((prev) => [
          ...
          {
            sender: "bot",
            text: "Innvikta offers tailored pricing plans based on organization size and specific simulation requirements. You can start a free trial with up to 50 users on our [Start Free page](/start-free). For detailed enterprise quotes and custom pricing sheets, feel free to [Book a Demo](/book-demo) or type 'book a demo' here to request a callback."
          }
        ]);
        return;
      }

      // Check general cybersecurity topics
      let reply = "";
      if (lowerText.includes("mfa fatigue") || lowerText.includes("prompt bomb")) {
        reply = "MFA fatigue (or prompt bombing) happens when hackers spam a victim with multi-factor authentication prompts until they tap 'approve' out of frustration or error. Innvikta InSAT features dedicated simulation modules and training lessons to teach employees how to handle MFA fatigue attacks.";
      } else if (lowerText.includes("ransomware") || lowerText.includes("encrypt")) {
        reply = "Ransomware is malicious software designed to lock your systems or encrypt files until a ransom is paid. The primary delivery method is phishing. Innvikta helps you run simulations to train employees to flag high-risk attachments and links that deliver ransomware payloads.";
      } else if (lowerText.includes("social engineering") || lowerText.includes("manipulat")) {
        reply = "Social engineering involves manipulating people into giving up confidential information (like passwords or bank details). Innvikta's simulations train users on various social engineering vectors including phishing (email), smishing (SMS), and vishing (voice calls).";
      } else if (lowerText.includes("mfa") || lowerText.includes("2fa") || lowerText.includes("auth")) {
        reply = "Multi-Factor Authentication (MFA) adds an essential layer of defense by requiring two or more verification steps. Implementing MFA reduces account compromise by 99%, and Innvikta includes modules teaching employees how to secure authorization codes and avoid credentials leakage.";
      } else if (lowerText.includes("vector")) {
        reply = "Innvikta simulates multiple **phishing vectors** to build complete employee resilience. These include:\n- **Email Phishing**: Fake login requests, secure document attachments, and urgent billing notifications.\n- **Smishing (SMS)**: False bank alerts, package delivery trackers, and MFA bypass attempts.\n- **Vishing (Voice)**: Audio social engineering, caller ID spoofing, and AI voice clone scams.\n- **QR Code Phishing (Quishing)**: Malicious QR codes embedded in emails or printed office posters.\n\nRead more details on our [Phishing Simulation solution page](/solutions/phishing-simulation).";
      } else if (lowerText.includes("landing") || lowerText.includes("hook")) {
        reply = "Our **Landing Page Hooks** simulate realistic login templates to test employee credential-harvesting resilience. When employees click a link, they land on cloned portals mimicking popular tools (such as Microsoft 365, Google Workspace, Salesforce, or corporate VPNs). If they submit entries, they are immediately guided to a secure micro-learning page. Read more on the [Phishing Simulation solution page](/solutions/phishing-simulation).";
      } else if (lowerText.includes("phish") || lowerText.includes("email") || lowerText.includes("link")) {
        reply = "Phishing is the practice of sending fraudulent messages designed to trick people into revealing sensitive information. Innvikta InSAT simulates these scenarios to build employee resilience. Check out our [Phishing Simulation solution](/solutions/phishing-simulation) to learn more.";
      } else if (lowerText.includes("insat") || lowerText.includes("workspace") || lowerText.includes("platform")) {
        reply = "InSAT (Integrated Security Awareness Training) is our central SaaS workspace. It automates campaign schedules, distributes micro-learning modules, and gathers human risk analytics. Read more on the [InSAT solution page](/solutions/insat).";
      } else if (lowerText.includes("roi") || lowerText.includes("calc")) {
        reply = "We have an interactive Security Awareness ROI Calculator that uses industry-specific click-rate benchmarks to calculate cost savings and risk score reductions. Try it now on our [Simulation ROI Calculator page](/resources/simulation-roi)!";
      } else if (
        ...
        reply = "If you are a victim of cybercrime, online scams, or financial fraud, please take these immediate steps:\n\n1. **Call 1930**: Call the National Cyber Crime Helpline (India) immediately to report financial fraud and freeze fraudulent transactions.\n2. **File a Complaint**: Submit a formal incident report on the official government website at [cybercrime.gov.in](https://cybercrime.gov.in).\n3. **Freeze Accounts**: Contact your banking institution immediately to freeze compromised cards, accounts, or UPI handlers.\n4. **Preserve Evidence**: Save transaction screenshots, receipts, headers, and chat messages.\n\nOur **[Innvikta Cyberhelp Portal](/cyberhelp)** provides detailed guided filing instructions. You can view our [Complaint Filing Guide](/cyberhelp/filing-guide) or use our [Incident Reporting Guide](/cyberhelp/register) for step-by-step assistance.";
      } else if (lowerText.includes("arcade") || lowerText.includes("game")) {
        reply = "Innvikta Arcade (or Cyber Arcade) is our interactive, game-based learning platform. It features high-fidelity cybersecurity games including:\n1. **Dash Car** (endpoint defense challenges)\n2. **Secure Inbox** (phishing email threat detection)\n3. **Crystal Planet** (sci-fi threat identification)\n4. **Escape Room** (puzzle-based security assessments)\n\nThese games turn compliance training into an active experience and improve employee security awareness retention by over 40%! You can explore the [Arcade Experience here](/cyber-arcade).";
      } else if (lowerText.includes("compliance") || lowerText.includes("soc2") || lowerText.includes("gdpr") || lowerText.includes("iso")) {
        reply = "Innvikta's **Compliance Training** enables your organization to satisfy audit requirements for standards like SOC2, GDPR, ISO 27001, HIPAA, and PCI-DSS. We deliver automated, interactive courses with full progress tracking, knowledge assessments, and policy signing tools. Read details on the [Compliance Training solution page](/solutions/compliance-training).";
      } else if (lowerText.includes("risk") || lowerText.includes("analytics") || lowerText.includes("profil")) {
        reply = "Our **Human Risk Intelligence** platform monitors threat behaviors, profiles employee actions, and calculates real-time risk scores. It helps administrators trigger automated campaign interventions for high-risk profiles. Find out more on our [Human Risk Intelligence page](/solutions/human-risk-intelligence).";
      } else if (lowerText.includes("benchmark") || lowerText.includes("reporting rate") || lowerText.includes("click rate")) {
        reply = "Our **Maturity Benchmarks** compile global phishing simulation metrics (like average click rates and report rates) across industries so you can compare employee security levels with your peers. See the statistics on our [Maturity Benchmarks report](/maturity-benchmarks).";
      } else if (lowerText.includes("partner") || lowerText.includes("msp") || lowerText.includes("reseller")) {
        reply = "Innvikta offers dedicated **Partner Programs** tailored for MSPs, MSSPs, and security consultants. Our program features multi-tenant portal management, co-branded marketing assets, and wholesale discount structures. Learn more on our [Partners page](/partners).";
      } else if (lowerText.includes("trial") || lowerText.includes("limit") || lowerText.includes("free tier") || lowerText.includes("setup")) {
        reply = "Our **Free Trial** lets you roll out Innvikta's full-featured workspace to up to 50 active users. Setup is instant and requires no credit card. Set up your dashboard now on our [Start Free page](/start-free).";
      } else if (lowerText.includes("tip") || lowerText.includes("trend") || lowerText.includes("campaign")) {
        reply = "Explore our latest cybersecurity blogs for social engineering updates, phishing awareness templates, and enterprise security training guides. Visit our [Threat Blog](/blog) for daily threat updates.";
      } else if (
        ...
        reply = `That is an important cybersecurity topic! Educating employees on terms like that is exactly what Innvikta does. Our InSAT platform contains interactive lessons covering passwords, malware, spoofing, and endpoint protection. Would you like to schedule a quick demo to see them?`;
      } else {
        ...
        {bookingState.step === "idle" && !isTyping && (
          <div className="px-3.5 py-1.5 bg-slate-50 border-t border-slate-100 flex flex-wrap gap-1.5">
            {quickPills.map((pill, pIdx) => (
              <button 
                key={pIdx}
                onClick={() => setInputValue(pill.query)}
